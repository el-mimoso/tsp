from memetic import *
# Load Data
data = pd.read_csv(r'benchmarks/usa13509.tsp', skiprows=[0, 1, 2, 3, 4, 5],
                   header=None, sep=' ')[:-1]
...
bi, gB, progress = geneticAlgorithm(
    population=cityList, popSize=50, eliteSize=25, generations=5, adjMat=adjMatrix)
tock = timer()
exeTime = tock-tick
print(f"time: {exeTime}")
# Save to File
saveFile(bestInitial=bi, globalBest=gB, progress=progress,
         timer=exeTime, title="usa13Test")
